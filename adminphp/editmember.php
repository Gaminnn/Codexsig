...
$stmt = $conn->prepare("UPDATE members SET name = ?, studentNumber = ?, email = ?, program = ?, year = ?, status = ?, participation = ?, points = ?" . ($password !== '' ? ", password = ?" : "") . " WHERE id = ?");
if (!$stmt) {
    http_response_code(500);
...
if ($password !== '') {
    $stmt->bind_param("ssssssdiis", $name, $studentNumber, $email, $program, $year, $status, $participationVal, $pointsVal, $password, $id);
} else {
    $stmt->bind_param("ssssssdii", $name, $studentNumber, $email, $program, $year, $status, $participationVal, $pointsVal, $id);
}
